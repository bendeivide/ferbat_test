...
SimMCCen1 <- function(n, r, mu = 100, cvp=0.10, alpha, N, two.sided = TRUE) {
  # Objeto que armazena a probabilidade do erro tipo I por experimento dos testes
  Result <- matrix(0, 1, 2)
  rownames(Result) <- paste("alpha", as.character(alpha),sep = " = ")
  colnames(Result) <- c("Dunnett", "Ferbat")
  
  # Probabilidade do erro tipo I por experimento de cada experimento
  add <- 1 / N
  dn <- function(r, np = 50){
    GL <- function(size) {
  ...
  # Funcao que computa o erro tipo I por experimento dos testes
  funaux <- function(teste1, teste2, Result, q1, q2){
    if (any(teste1 >= q1)) {
  ...
  # Simulacao do experiment DIC em cinco cenarios
  ...
  # Simulacao do erro tipo I por experimento em paralelo dos testes
  aux <- foreach(i = 1:N, .combine = "+") %dopar% {
    Y <- SimulateData(n = n, r = r, delta = 0, cenario = 1)
    ...
    # Resultado
    funaux(teste1, teste2, Result, q1[posn, posr], q2[posn, posr])
  }  
  return(aux)
}